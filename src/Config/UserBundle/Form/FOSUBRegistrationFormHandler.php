...
namespace Config\UserBundle\Form;
...
class FOSUBRegistrationFormHandler  implements RegistrationFormHandlerInterface
{
    ...
    public function process(Request $request, Form $form, UserResponseInterface $userInformation)
    {
        ...
        $fb_user = $userInformation->getResponse();

        $random_string = $this->generateRandomString();

        $facebook_id = $fb_user['id'];
       
        $image = $fb_user['picture']['data']['url'];

        $password = $encoder->encodePassword($random_string.$facebook_id, $user->getSalt());
        

        $user->setEnabled(true);
        $this->setUserInformation($user,$userInformation);
